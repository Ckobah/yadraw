...
export function registerV2Routes(server: FastifyInstance, service: V2BoardService): void {
  ...
  server.patch("/v2/boards/:boardId/card-types/:cardTypeId/schema", async (request, reply) => {
    try {
      ...
      return await service.updateCardTypeSchema(
        request.requestContext,
        boardId,
        cardTypeId,
        request.body as any
  ...
  server.post(
    "/v2/workspaces/:workspaceId/card-types/:cardTypeId/library-entries/workbook/export",
    ...
    async (request, reply) => {
      try {
        ...
        return await service.exportXlsxLibraryWorkbook(
          request.requestContext,
          workspaceId,
          cardTypeId,
          request.body as any
  ...
  server.post(
    "/v2/workspaces/:workspaceId/card-types/:cardTypeId/library-entries/imports/xlsx/preview",
    { config: { rateLimit: { max: 20, timeWindow: "1 minute" } } },
    async (request, reply) => {
      try {
        const { workspaceId, cardTypeId } = request.params as {
        ...
        return await service.previewXlsxLibraryImport(
          request.requestContext,
          workspaceId,
          cardTypeId,
          request.body as any
  ...
  server.post(
    "/v2/workspaces/:workspaceId/card-types/:cardTypeId/library-entries/imports/xlsx/commit",
    { config: { rateLimit: { max: 10, timeWindow: "1 minute" } } },
    async (request, reply) => {
      try {
        const { workspaceId, cardTypeId } = request.params as {
