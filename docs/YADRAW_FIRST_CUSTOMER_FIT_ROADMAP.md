...
Статус: реализация продолжается; выполняется этап 2A.
...
- этап 2A: реализован ограниченный транзакционный CSV import в библиотеку выбранного card type с
  mapping, preview и явными create/skip/update режимами; production и prospect validation gate ещё
  не пройдены.
...
## Этап 2. CSV import в библиотеки и практичный tabular export

Цель: убрать ручной повторный ввод для process, org-chart, roadmap и metadata-heavy сценариев.

### Этап 2A. Импорт записей библиотеки из CSV

- Принимать UTF-8 CSV с документированным лимитом размера и количества строк.
- Импортировать в библиотеку выбранного существующего card type. Первая версия не создаёт card type
  и не размещает карточки на canvas: пользователь один раз загружает canonical records, затем
  переиспользует их на нужных досках.
- До mutation показывать preview: headers, mapping, invalid rows и точное количество создаваемых,
  обновляемых и пропускаемых записей.
...
- Update меняет только явно mapped columns и сохраняет остальные поля canonical record.
- Импортировать ограниченным транзакционным batch с rollback при invalid row, ambiguous match или
  изменившемся после preview состоянии.
- Первая версия не импортирует attachments, не создаёт cards и не угадывает connections.
...
- импортированные library records можно переиспользовать на разных досках;
- Unicode сохраняется, CSV корректно открывается spreadsheet tools;
- JSON export и attachment boundaries не меняются.

### Validation gate

Один process-map prospect импортирует реальную или безопасно анонимизированную таблицу в библиотеку
и переиспользует минимум одну запись на доске без повторного ввода её полей.
